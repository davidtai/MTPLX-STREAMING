...
def _payload(freq, power, gpu_busy, cpu_busy, gpu_c, cpu_c, *, busy_key="gpu_usage_ratio"):
    return {
        "gpu_freq_mhz": freq,
        "gpu_power": power,
        busy_key: gpu_busy,
        "cpu_usage_ratio": cpu_busy,
...
def test_parse_extracts_fields():
    p = _payload(1030.0, 8.2, 0.45, 0.18, 71.0, 77.0)
    s = um.parse_macmon_payload(p)
    assert s["gpu_freq_mhz"] == 1030.0
    assert s["gpu_power_w"] == 8.2
    assert s["gpu_busy_ratio"] == 0.45
    assert s["cpu_busy_ratio"] == 0.18
    assert s["gpu_temp_c"] == 71.0
    assert s["cpu_temp_c"] == 77.0


def test_parse_busy_ratio_fallback_field_name():
    # a macmon build that spells it gpu_active_ratio still lands
    p = _payload(900.0, 6.0, 0.33, 0.10, 60.0, 65.0, busy_key="gpu_active_ratio")
    s = um.parse_macmon_payload(p)
    assert s["gpu_busy_ratio"] == 0.33


def test_parse_missing_fields_are_zero():
    s = um.parse_macmon_payload({})
    for k in ("gpu_freq_mhz", "gpu_power_w", "gpu_busy_ratio", "gpu_temp_c"):
        assert s[k] == 0.0


# --- summarize -------------------------------------------------------------
def test_summarize_min_mean_max_and_series():
    samples = [um.parse_macmon_payload(_payload(f, 8.0, 0.4, 0.2, 70.0, 76.0))
    ...
    assert set(summ["series"][0]) >= {"gpu_freq_mhz", "gpu_power_w", "gpu_busy_ratio", "t"}
...
def test_summarize_drop_series():
    samples = [um.parse_macmon_payload(_payload(1000.0, 8.0, 0.4, 0.2, 70.0, 76.0))]
...
# --- census ----------------------------------------------------------------
def test_census_line_format():
    samples = [um.parse_macmon_payload(_payload(1030.0, 8.2, 0.45, 0.18, 71.0, 77.0))]
...
def test_sampler_with_injected_lines():
    lines = [
        json.dumps(_payload(580.0, 8.0, 0.30, 0.15, 70.0, 75.0)),
        "not json - must be skipped",
        json.dumps(_payload(1381.0, 9.0, 0.60, 0.20, 72.0, 78.0)),
    ]
