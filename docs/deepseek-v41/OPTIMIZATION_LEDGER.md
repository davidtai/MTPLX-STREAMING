...
**2026-09-13 evidence correction:** native depth5 MTP now measures 8.556267 TPS
on the pinned 16K/1K Python workload under 110 GB, versus 5.790674 TPS for its
same-capacity AR reference. Its larger expert union increases bytes per output
but amortizes other work. The former blanket K>3 veto from adjacent models is
therefore withdrawn; no current depth3-vs-depth5 comparison has been measured.
See [the full receipt](receipts/dspark-cache-owned-20260913/README.md).
...
| **mxfp4 bank lossless compression as the next route to 20 TPS** | [Twelve real records](receipts/mxfp4-entropy-20260913/README.md) compress 1.10072x with zlib1 (9.15% fewer bytes); ideal byte-entropy savings are 6.76% per whole record or 10.75% with separate component models. This supersedes the earlier unmeasured ~1.0x forecast. The gain cannot close the 20 TPS gap alone, and the existing streamed decoder serializes records and adds host copies. Deprioritized pending a direct-slot decoder and separate scratch/throughput proof; not a claim that these bytes are incompressible. |
