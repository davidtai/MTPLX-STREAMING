...
round_page=lambda value: ((value + page - 1)//page)*page
allocated_growth=sum(round_page(3*seg.length)-round_page(2*seg.length) for seg in record.segments)
assert added==18800640
assert active-initial==allocated_growth
bank.close();mx.synchronize();mx.clear_cache()
assert mx.get_active_memory() < 1024**2
report={'scope':'single native six-component bank, 2 to 3 rows; byte copies and release accounting, no model generation','initial_active_bytes':initial,'grown_active_bytes':active,'peak_active_bytes':peak,'active_after_close_bytes':mx.get_active_memory(),'added_bytes':added,'allocated_growth_bytes':allocated_growth,'page_size_bytes':page,'growth_seconds':elapsed,'row_sha256':hashes,'allocator_limit_bytes':512*1024**2,'allocator_cache_request_bytes':64*1024**2,'helper_sha256':hashlib.sha256((root/'bank_growth_final.py').read_bytes()).hexdigest(),'manifest_sha256':meta['manifest_sha256']}
(root/'probe-final-results.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report),flush=True)
