...
def grow_bank(bank, capacity, *, mx, release_slack_bytes=1024**2):
    ...
    for component in tuple(bank.arrays):
        ...
        before_sync = mx.get_active_memory()
        mx.synchronize()
        after_sync = mx.get_active_memory()
        print({'component': component, 'expected': initial_active + added, 'before_sync': before_sync, 'after_sync': after_sync}, flush=True)
        if after_sync > initial_active + added + release_slack_bytes:
            raise RuntimeError('old component backing remains live after synchronization')
