...
signal.alarm(900)
PREFIX=pathlib.Path('/tmp/dsv41-110-preflight/python-16k-1024-reclaimed-defaults-v2')
...
if not 75 <= slots <= 84: raise RuntimeError(f'unexpected slot geometry: {slots}')
...
bounds={'source_commit':source_commit,'wrapper_sha256':hashlib.sha256(pathlib.Path(__file__).read_bytes()).hexdigest(),
 ...
 'scope':'same 16K/1023-step AR pf0 forward geometry; exact persistent delta beyond conservative 72-slot peak projection; full 2GiB allocator cache and 2GiB Python capacity; 10GiB transient reserve'}
...
try:
 ...
 if (args.arms!=['cell16k_ring_v2_attn_pf0'] or args.decode_mode!='ar' or args.context_tokens!=16384
     or args.decode_tokens!=1023 or args.max_kv!=17664 or args.transient_band_gib!=10
