"""Exact full workload with a measured-peak-bounded 6 GiB transient plan band."""
import hashlib
...
signal.alarm(1200)
PREFIX = Path('/tmp/dsv41-110-preflight/python-16k-1024-band6')
GIB = 1024**3
RECORD = 18800640
FIXED_MTP = 31931661128
MTP_ADDITIONAL = 8353408392
MTP_PEAK = 87384896236
BASE_SLOTS = 72
GRAPH_MARGIN = 2 * GIB
...
prior = json.loads(Path('docs/deepseek-v41/receipts/dspark-layer-prefill-20260913/summary.json').read_text())
...
# The previous COMPLETE run bounds AR, MTP and replay. Charge the exact added
# expert storage plus 2 GiB graph/workspace variation. Keep 2 GiB Python and
# 2 GiB allocator cache outside active Metal, with zero prefill-cache credit.
# The 6 GiB planning band still exceeds the measured active overshoot and
# contains the 2 GiB allocator-cache allowance; this change is scoped here.
engine = int(110e9 - base - 2 * GIB - 6 * GIB)
slots = (engine - FIXED_MTP) // (40 * RECORD)
assert 72 <= slots <= 80
active_bound = MTP_PEAK + (slots - BASE_SLOTS) * 40 * RECORD + GRAPH_MARGIN
prior_full_physical_bound = prior['physical_peak_bytes'] + base - 9432500000 + (slots - BASE_SLOTS) * 40 * RECORD + GRAPH_MARGIN
...
bounds = {
    ...
    'scope': 'exact native artifact,16K prompt,1023 steps,depth5,pf0,eager target attention,compiled draft,bf16 heads; 6GiB planning band,2GiB cache,2GiB Python',
}
...
try:
    ...
    if (Path(args.model).resolve() != Path('/Users/davidtai/models/DeepSeek-V4.1-Flash-MTPLX-streaming-mxfp4')
        ...
        or args.max_kv != 17664 or args.box_target_gb != 110 or args.transient_band_gib != 6
