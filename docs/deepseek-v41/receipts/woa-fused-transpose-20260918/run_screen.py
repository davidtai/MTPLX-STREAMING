...
files = [Path(installation['model_path']) / name for name in installation['shards']]
...
try:
    ...
    print('ATTENTION_SOURCE_CACHE_RECLAIMED', json.dumps({k:v for k,v in report.items() if k != 'files'}), flush=True)
