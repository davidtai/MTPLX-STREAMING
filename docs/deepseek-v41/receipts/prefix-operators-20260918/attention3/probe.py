...
def run():
 ...
 for layers in proof['cases']:
  ...
  def fresh():
   ...
   return caches
  batches={}
  for mode,widths in (('full6',(6,)),('prefix3_tail3',(3,3))):
   start=0;chunks=[]
   for width in widths:
  ...
  for mode in ('full6','prefix3_tail3','full6','prefix3_tail3','full6'):
   totals=[];first=[]
   for _ in range(7):
  ...
  native,nc,ns,_,_=execute('full6',True);candidate,cc,cs,_,_=execute('prefix3_tail3',True)
  parity={}
  for l in layers:
  ...
  result['arm_median_ns']=meds;result['first_part_median_ns']=first;result['total_cost_ratio']=meds['prefix3_tail3']/meds['full6'];result['prefix_available_earlier_ns']=meds['full6']-first['prefix3_tail3']
