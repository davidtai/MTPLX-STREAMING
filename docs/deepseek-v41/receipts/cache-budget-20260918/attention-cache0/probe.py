...
def select_mode(mode):
 mx.synchronize();mx.set_cache_limit(0 if mode=='nocache' else 1024**3);mx.clear_cache()
...
def run():
 ...
 for layers in proof['cases']:
  ...
  def fresh():
   ...
   return caches
  batches={}
  for mode,widths in (('full6',(6,)),('nocache',(6,))):
   start=0;chunks=[]
   for width in widths:
  ...
  for mode in ('full6','nocache','full6','nocache','full6'):
   select_mode(mode)
   totals=[];first=[];cached=[]
   for _ in range(7):
    _,_,_,parts,total=execute(mode);totals.append(total);first.append(parts[0]);cached.append(int(mx.get_cache_memory()))
    if mode=='nocache' and cached[-1]!=0:raise RuntimeError('disabled cache retained buffers')
  ...
  select_mode('nocache');candidate,cc,cs,_,_=execute('nocache',True)
  parity={}
  for l in layers:
  ...
  result['arm_median_ns']=meds;result['first_part_median_ns']=first;result['total_cost_ratio']=meds['nocache']/meds['full6']
