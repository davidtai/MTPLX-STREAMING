...
def run():
    ...
    restored = type(layer).from_state(snapshot, layer.meta_state)
    mx.eval(restored.eval_backing())
    for actual, expected in ((restored.window, old_window), (restored.compress_kv, old_compress), (restored.index_k, old_index)):
        if not np.array_equal(np.array(actual), expected):
            raise RuntimeError('packed snapshot restore changed Q8 prefix bytes')
    report['checks'].append('packed snapshot restore preserves quantization codes and metadata')
    ...
    native_draft = DSparkStageCache(window_size=128, head_dim=512)
    native_draft.append_main(window_reference)
    mx.eval(native_draft.detach_prefill_backings())
    if not np.array_equal(np.array(native_draft.window), np.array(window_reference[:, -128:])):
        raise RuntimeError('native draft prefill detach changed its window')
    report['checks'].append('native draft prefill detach preserves its floating window')
...
def run_tiny_model():
    ...
    from mtplx.models.deepseek_v41_fixed_q8_cache import install_fixed_q8_cache
    args = ModelArgs(vocab_size=64, hidden_size=64, num_hidden_layers=4,
        num_attention_heads=2, head_dim=64, qk_rope_head_dim=8, q_lora_rank=16,
        o_lora_rank=16, o_groups=1, moe_intermediate_size=64, n_routed_experts=4,
        num_experts_per_tok=2, index_n_heads=2, index_head_dim=64, index_topk=16,
        sliding_window=8, window_size=8, swiglu_limit=0.5,
        compress_ratios=[0,2,2,1], kv_source_layer_ids=[1,3],
        index_source_layer_ids=[1,3], candidate_source_layer_id=3,
        candidate_topk_blocks=8, candidate_block_size=2)
    mx.random.seed(17)
    model = Model(args)
    mx.eval(model.parameters())
    native = model.make_cache()
    installed = install_fixed_q8_cache(model, max_kv=96, max_append=16)
    fixed = model.make_cache()
    ids = mx.array([list(range(32))], mx.int32)
    a = model(ids, cache=native, prefill_chunk=16)
    mx.eval(a)
    b = model(ids, cache=fixed, prefill_chunk=16)
    mx.eval(b)
    aa, bb = np.array(a), np.array(b)
    if not np.isfinite(bb).all():
        raise RuntimeError('fixed Q8 tiny-model logits are not finite')
    report['tiny_model_prefill'] = dict(max_abs=float(np.max(np.abs(aa-bb))),
        relative_rms=float(np.linalg.norm(aa-bb)/np.linalg.norm(aa)),
        greedy_rows_equal=int(np.sum(aa.argmax(-1)==bb.argmax(-1))), rows=32,
        installed=installed)
    allocated = sum(x.nbytes for layer in fixed for x in layer.eval_backing())
    for cycle in range(12):
        block = mx.array([[(cycle+j)%64 for j in range(6)]], mx.int32)
        y = model(block, cache=fixed)
        mx.eval(y)
        if not np.isfinite(np.array(y)).all():
            raise RuntimeError('fixed Q8 tiny-model verify logits are not finite')
        if fixed.trim(5) != 5:
            raise RuntimeError('fixed Q8 tiny-model verify trim lost coverage')
    if fixed.offset != 44 or sum(x.nbytes for layer in fixed for x in layer.eval_backing()) != allocated:
        raise RuntimeError('tiny-model KV offset or fixed storage changed unexpectedly')
    try:
        fixed.assert_can_admit(53)
    except ValueError:
        pass
    else:
        raise RuntimeError('fixed context overflow was admitted')
    report['checks'].append('tiny real model: prefill, 12 verify/trim cycles, fixed storage and context overflow')
...
if report['mlx_active_after_close_bytes'] > 2 * 1024**2:
    raise RuntimeError('cache owners retained Metal storage after request teardown')
