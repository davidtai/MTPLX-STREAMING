...
def load_projection_tensors(root, manifest, kept, *, mx):
    if len(kept) != 4 or sum(t.length for t in kept) != 77856768:
    ...
    if set(grouped) != {'model-00003.safetensors'}:
    ...
    with ExitStack() as stack:
        for name, tensors in grouped.items():
            ...
            if not stat.S_ISREG(original.st_mode) or original.st_size > 175000000:
