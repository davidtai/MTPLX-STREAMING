...
GROWTH_ROOT = Path('/tmp/dsv41-io-fanout8-20260917')
...
if os.environ.get('MTPLX_DSV41_IO_READ_FANOUT') != '8' or os.environ.get('MTPLX_BELADY_ORACLE','0') != '0':
    raise RuntimeError('fanout8 and no route instrumentation required')
if not GROWTH_ENABLED:
    raise RuntimeError('fanout8 comparison requires the fixed cap93 to cap100 transition')
...
bounds = {
 ...
 'io_read_fanout':8, 'fanout_pool_workers':35,
...
try:
    ...
    def checked_load(*a, **kw):
        ...
        if (rt.reader.io_read_fanout != 8 or rt.reader._fanout_pool_workers != 35
            or not rt.spec.mtp_included or rt.plan.slots_per_layer != slots
    ...
    def run_arm_with_reference(*a, **kw):
        ...
        receipt['io_fanout_comparison'] = {'read_fanout':8, 'fanout_pool_workers':35,
            'unchanged_record_bytes':18800640, 'unchanged_shared_transient_slots':48,
            'additional_io_host_headroom_bytes':growth_admission['additional_io_host_headroom_bytes'],
            'control_receipt':'/tmp/dsv41-110-stage/full-cache-growth-d405-20260917.jsonl'}
