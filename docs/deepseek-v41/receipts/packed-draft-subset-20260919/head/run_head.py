...
for name in ('head-screen.json', 'child.json', 'reclamation.json'):
    if (ROOT / name).exists():
...
child = subprocess.Popen([sys.executable, str(ROOT / 'head_screen.py')], env=env)
...
try:
    ...
    print('HEAD_SOURCE_CACHE_RECLAIMED', json.dumps({k:v for k,v in report.items() if k != 'files'}), flush=True)
