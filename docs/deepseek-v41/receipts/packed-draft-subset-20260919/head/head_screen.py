...
OUT=Path('/private/tmp/dsv41-packed-subset-20260919/head/head-screen.json')
...
CURRENT_ROOT=Path('/private/tmp/dsv41-packed-subset-20260919/head')
...
del weights['full']
gc.collect();mx.synchronize();mx.clear_cache()
draft_config = json.loads((CURRENT_ROOT/'draft-config.json').read_text())
original_luts = LUTS
candidate_luts = []
...
for sequence, mode in enumerate(('hybrid_m8','packed_subset','hybrid_m8')):
    ...
    print('PACKED_SUBSET_ARM',json.dumps({k:row[k] for k in ('mode','cycles','verify_rows','verify_width_counts','head_replay_wall_s','mlx_peak_bytes')}),flush=True)
