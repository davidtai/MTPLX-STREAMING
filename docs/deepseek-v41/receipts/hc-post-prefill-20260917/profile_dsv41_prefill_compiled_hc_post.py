...
COMPATIBILITY = json.loads(Path('/tmp/dsv41-110-stage/hc-post-prefill-source-compatibility-ca207-20260917.json').read_text())
...
_prefill_peak_samples = []
_prefill_high_water = 0

def _prefill_mem_probe(stage, layer=-1, chunk=-1):
    global _prefill_high_water
    peak = int(mx.get_peak_memory())
    if peak > _prefill_high_water or stage != 'attention_chunk':
        row = {'stage': stage, 'layer': int(layer), 'chunk': int(chunk),
               'active_bytes': int(mx.get_active_memory()),
               'cache_bytes': int(mx.get_cache_memory()), 'peak_bytes': peak}
        _prefill_peak_samples.append(row)
        _prefill_high_water = max(_prefill_high_water, peak)

dsv41._prefill_mem_probe = _prefill_mem_probe
_candidate_prefill_source = _installed_prefill_source.replace(
    _fence, _fence + "\n                _prefill_mem_probe('attention_chunk', layer.layer_id, c)")
_moe_call = 'moe_outputs = self._layer_major_moe(layer, moe_inputs, spans, row_cap)'
assert _candidate_prefill_source.count(_moe_call) == 1
_candidate_prefill_source = _candidate_prefill_source.replace(_moe_call,
    "_prefill_mem_probe('before_moe', layer.layer_id)\n        " + _moe_call +
    "\n        _prefill_mem_probe('after_moe', layer.layer_id)")
assert _candidate_prefill_source.count('mx.eval(hs)') == 1
_candidate_prefill_source = _candidate_prefill_source.replace('mx.eval(hs)',
    "mx.eval(hs)\n        _prefill_mem_probe('after_combine_fence', layer.layer_id)")

# One direct prefill-only installation. Preserve the original einsum and
# existing layer fence; no input retirement, extra evaluation or hot fallback.
_combine = 'hs[c] = layer.moe_combine(moe_outputs[c], carries[c])'
if _candidate_prefill_source.count(_combine) != 1:
    raise RuntimeError('prefill HC post call changed')
dsv41._prefill_hc_post = mx.compile(dsv41._hc_post_impl)
_candidate_prefill_source = _candidate_prefill_source.replace(
...
ACTIVE_ESTIMATE_MARGIN_BYTES = 512 * 1024**2
for admitted_slots in range(TARGET_SLOTS, 0, -1):
    ...
    candidate_physical = (CONTROL_EXTERNAL_SYSTEM_PEAK_BYTES + delta
                          + 1_000_000_000 + max(0, base - CONTROL_BASELINE_BYTES))
...
physical_bound = projected_physical_peak + 1_000_000_000 + baseline_drift_bytes
...
try:
    ...
    # Apply these at the construction boundary through the corrected runner.
    # The receipt reports this explicit override instead of inferring engagement.
    ...
    def checked_load(*a, **kw):
        ...
        _loaded_args = a[0] if a else kw['args']
        if _loaded_args._dsv41_bound_model_levers != _bound_prefill_flags:
            raise RuntimeError('prefill construction did not bind the explicit flags')
        if dsv41._HC_COMPILE or dsv41._ATTN_COMPILE or dsv41._ATTN_WIN_MEMO:
            raise RuntimeError('prefill construction flags differ')
    ...
    if {k: v for k, v in reference.get('arm_env', {}).items() if k not in _bound_prefill_flags} != {
        ...
        raise RuntimeError('prefill environment differs outside the three explicit flags')
    ...
    # One observation outside the decode clock identifies whether the overall
    # allocator peak has already happened during prefill. Do not reset the peak.
    ...
    def dspark_with_boundary_observation(*a, **kw):
        ...
        def observe_prefill_boundary(info):
            ...
            _prefill_mem_probe('prefill_complete')
            profile = {
                'prefill_hc_post_compiled': True,
                'bound_model_levers': dict(_bound_prefill_flags),
                'scope': COMPATIBILITY['scope'],
                'source_commit': source, 'wrapper_sha256': hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
                'source_compatibility': COMPATIBILITY,
                'baseline_bytes': base, 'slots_per_layer': slots,
                'samples': _prefill_peak_samples,
                'prefill_seconds': info.get('prompt_eval_time_s'),
                'boundary_memory': prefill_boundary_memory,
            }
            PREFIX.with_suffix('.prefill-profile.json').write_text(json.dumps(profile, indent=2) + '\n')
            print('PREFILL_PEAK_PROFILE_COMPLETE', json.dumps(prefill_boundary_memory), flush=True)
            raise SystemExit(0)
    ...
    ab.main()
    raise RuntimeError('prefill-only boundary failed to stop before decode')
finally:
    stop.set()
    thread.join(2)
    sample()
