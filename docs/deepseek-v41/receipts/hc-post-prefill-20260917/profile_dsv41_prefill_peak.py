...
COMPATIBILITY = json.loads(Path('/tmp/dsv41-110-stage/retire-input-source-compatibility-20260917.json').read_text())
...
exec(compile(_candidate_prefill_source, '<stage-retire-consumed-input>', 'exec'),
...
if admitted_slots != 94:
    raise RuntimeError('live baseline cannot admit the matched cap94 capacity')
...
process_footprint_bound = CONTROL_PROCESS_PEAK_BYTES + slot_delta_bytes
wired = host_memory_snapshot()['box']['wired_bytes']
if (
...
bounds = {
    ...
    'retained_allocator_cache_bound_bytes': GIB,
    ...
    'process_footprint_bound_bytes': process_footprint_bound,
    ...
    'scope': 'exact native artifact,16K prompt,bounded depth3/4/5,pf0,trace-covered compact resident MTP experts,stock target attention,compiled draft,bf16 heads; corrected fixed-footprint slot plan,1GiB cache,2GiB Python',
}
...
try:
    ...
    if reference.get('arm_env') != expected_arm_env:
        raise RuntimeError('AR reference target arithmetic environment differs')
    ...
    def dspark_with_boundary_observation(*a, **kw):
        ...
        def observe_prefill_boundary(info):
            ...
            profile = {
                'scope': 'prefill-only allocation profile at existing fences; no extra evals; exits before decode; not a throughput receipt',
                'source_commit': source, 'wrapper_sha256': hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
                'source_compatibility': COMPATIBILITY,
    ...
    def cached_ar_logits_row(**kw):
        ...
        if data_path.exists() or meta_path.exists():
            ...
            row = original_ar_logits_row(**kw)
            buffer = io.BytesIO()
            ab.np.save(buffer, row, allow_pickle=False)
            blob = buffer.getvalue()
            metadata = {**expected, 'npy_sha256': hashlib.sha256(blob).hexdigest()}
            if row.shape != (129280,) or row.dtype != ab.np.dtype('float32'):
                raise RuntimeError('AR diagnostic row shape or dtype differs')
