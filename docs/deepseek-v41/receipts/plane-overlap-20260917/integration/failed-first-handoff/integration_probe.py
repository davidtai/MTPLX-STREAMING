...
from mtplx.models.expert_mlx import make_mlx_component_bank_allocator, HotExpertSwitchGLU
...
def arm(mode, sequence):
    ...
    try:
        ...
        if sum(b.capacity for b in allocator.banks.values()) != 56:
        ...
        if mode=='stock':
            ...
            switch._dispatch_component_bank=make_dispatch(scales,mx=mx)
        else:
            runners=plane_lane.install(runtime,{20:switch},{20:scales},early=mode=='early')
        rng=np.random.default_rng(202641)
        permutation=rng.permutation(384)
        # Two identical adjacent calls cover persistent hits at M1 and physical
        # transient reuse at M6; changing groups produces real split misses.
        for rows in (1,6):
            ...
            for call in range(12):
                start_expert=24*(call//2)
                ids=np.array([[permutation[(start_expert+2*r+k)%384] for k in range(6)] for r in range(rows)],dtype=np.int32)
...
try:
    ...
    for sequence,mode in enumerate(('stock','full','early','stock','early','full','stock')):
        arm(mode,sequence)
