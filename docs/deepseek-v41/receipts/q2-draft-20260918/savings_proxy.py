...
rows = []
for policy, capacity in ((None, 73), ('transition-window', 110),
                         ('transition-window', 112), ('transition-window', 113)):
    counts = []
    for layer in range(40):
        bank = restore(data['initial_banks'][str(layer)],
                       policy=policy, single_pool=True, layer_id=layer)
    ...
    rows.append({'capacity': capacity, 'policy': bank.cache_policy,
                 'reads': reads, 'reads_by_layer': counts,
