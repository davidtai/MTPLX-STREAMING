...
OUT=Path('/private/tmp/dsv41-q2-draft-20260918/head-screen.json')
...
CURRENT_ROOT=Path('/private/tmp/dsv41-q2-draft-20260918')
...
class DraftOnly(nn.Module):
    def __init__(self,width,compact,expert_bits=4):
        super().__init__()
...
# Retire the unused full128 native bank before allocating Q2 compact storage.
del weights['full']
gc.collect(); mx.synchronize(); mx.clear_cache()
...
for mode in ('hybrid_control', 'q2_experts'):
    ...
    if mode == 'hybrid_control' and commit_lengths != control_lengths:
        raise RuntimeError('control differs from pinned198-cycle hybrid trajectory')
    row={'mode':mode,'cycles':len(commit_lengths),
    ...
    print('Q2_DRAFT_ARM',json.dumps({k:row[k] for k in ('mode','cycles','verify_rows','verify_width_counts','head_replay_wall_s','mlx_peak_bytes')}),flush=True)
