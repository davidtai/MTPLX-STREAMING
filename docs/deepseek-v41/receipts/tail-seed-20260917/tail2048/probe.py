...
def run():
    ...
    for arm in ('full', 'tail', 'full', 'tail'):
        ...
        first = 0 if arm == 'full' else 16384 - 2048
        ...
        row = {'arm': arm, 'seed_seconds': elapsed, 'peak_bytes': int(mx.get_peak_memory()),
               ...
               'state_dtypes': [str(main_h.dtype)] + [str(c.window.dtype) for c in caches],
...
report = {'scope': 'Native dense seed weights only; synthetic16K sequence from captured native hidden rows. Compares full seeding with only the final2048 rows at correct absolute offsets. No target/draft generation or TPS claim.',
