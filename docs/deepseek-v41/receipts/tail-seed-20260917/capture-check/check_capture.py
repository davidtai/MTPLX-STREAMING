    ...
    for i, layer in enumerate(cache.layers):
        for name in ('window', 'compress_kv', 'index_k'):
            ...
            if array is not None:
                out[f'{i}.{name}'] = bits(array)
        state = getattr(layer, 'comp_state', None)
        if state is not None:
            for name in ('raw_kv', 'raw_score'):
                array = getattr(state, name)
                if array is not None:
                    out[f'{i}.cs.{name}'] = bits(array)
...
def run():
    for dtype in (mx.float32, mx.bfloat16):
        ...
        control_logits = bits(logits)
        control_hidden = bits(hidden)
        control_state, control_offset = snapshot(control_cache)
        control_next = model(mx.array([[3]]), cache=control_cache)
        mx.eval(control_next)
        next_bits = bits(control_next)
        ...
        for keep in (7, 17):
            ...
            logits, hidden = model(ids, cache=cache, return_hidden=True,
                                   prefill_chunk=7, prefill_layer_major=True)
            mx.eval(logits, hidden)
            state, offset = snapshot(cache)
            equal_logits = exact(bits(logits), control_logits)
            equal_hidden = exact(bits(hidden), control_hidden[:, -keep:])
            ...
            mx.eval(nxt)
            equal_next = exact(bits(nxt), next_bits)
