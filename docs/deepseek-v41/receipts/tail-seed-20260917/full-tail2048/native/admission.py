...
ROOT = Path('/tmp/dsv41-tail-seed-20260917/full-v2/native')
...
def resolve_admission(base, wired, *, grow, expected_receipt_hash):
    ...
    margin = 256 * 1024**2
    padding = 40 * 6 * 16384
    host_reserve = 2 * GIB + 16 * 1024**2  # bound tail-capture Python metadata in every phase
    ...
    return dict(physical_ceiling_bytes=DEFAULT_BOX_BUDGET_BYTES, prefill_slots_per_layer=initial, decode_slots_per_layer=capacity,
        ...
        prefill_cache_allowance_bytes=prefill_cache, host_reserve_bytes=host_reserve, tail_capture_host_allowance_bytes=16 * 1024**2,
