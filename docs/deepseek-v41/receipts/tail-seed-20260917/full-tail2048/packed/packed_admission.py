...
def resolve_admission(base, wired, *, grow, expected_receipt_hash):
    ...
    spec = importlib.util.spec_from_file_location('native_growth_admission', '/tmp/dsv41-tail-seed-20260917/full-v2/native/admission.py')
    ...
    result.update(
        ...
        bound_scope='proved cap84 prefill and native M6/M8 envelope, all original allowances plus 16MiB host metadata for tail capture; exact packed storage and 64MiB early plane intermediates; no seed-memory discount',
    )
    return result
