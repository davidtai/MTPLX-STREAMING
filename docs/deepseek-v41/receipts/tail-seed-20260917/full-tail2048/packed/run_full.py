...
PACKED_ROOT = Path('/tmp/dsv41-tail-seed-20260917/full-v2/packed')
...
GROWTH_ROOT = Path('/tmp/dsv41-tail-seed-20260917/full-v2/compat')
...
_native_prefill_source = textwrap.dedent(inspect.getsource(dsv41.DeepseekV41Backbone._forward_layer_major))
if _native_prefill_source.count('_PREFILL_HC_POST(moe_outputs[c], *carries[c])') != 1:
...
bounds = {
 ...
 'native_prefill_sha256':hashlib.sha256(_native_prefill_source.encode()).hexdigest(),
 'candidate_prefill_sha256':hashlib.sha256((PACKED_ROOT / 'tail_prefill_body.py').read_bytes()).hexdigest(),
...
try:
    ...
    def checked_load(*a, **kw):
        global growth_transition, growth_report, prefill_resolved_plan, tail_prefill_report
