...
def run():
    ...
    original = loader.partition_text_residents(manifest, with_mtp=True)
    kept = tuple(t for t in original.kept if t.tensor in wanted)
    ...
    partition = replace(original, kept=kept, kept_count=len(kept), kept_bytes=sum(t.length for t in kept))
    raw = loader.load_text_only_resident_arrays(MODEL, manifest, mx_module=mx, partition=partition)
    ...
    del raw, weights, manifest, original, partition
