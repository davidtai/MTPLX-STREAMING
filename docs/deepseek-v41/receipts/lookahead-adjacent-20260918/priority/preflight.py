...
p['helper_sha256']={f.name:hashlib.sha256(f.read_bytes()).hexdigest() for f in r.iterdir()
    if f.is_file() and f.name in ('probe.py','priority_reads.py','predictor_cost.py','paired_config.py','plane_lane.py','packed_storage.py','paired_kernels.py','kernels.py','restore_bank.py','library_identity.py','run_screen.py','routes.json')}
