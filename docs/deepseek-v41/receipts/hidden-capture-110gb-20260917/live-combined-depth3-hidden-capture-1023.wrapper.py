...
# Evaluate the already-required MTP captures at the existing chunk fence.
# This releases each large HC source after its layer instead of letting the
# deferred mean retain it until the end of the entire layer-major prefill.
import inspect
import textwrap
_prefill_source = textwrap.dedent(inspect.getsource(
    dsv41.DeepseekV41Backbone._forward_layer_major
))
_old_fence = 'self._eval_layer_transients(lc, moe_in_c, ffn_pre_c)'
_new_fence = 'self._eval_layer_transients(lc, moe_in_c, ffn_pre_c, main_hiddens[c])'
if _prefill_source.count(_old_fence) != 1:
    raise RuntimeError('the exact target prefill fence has changed')
_prefill_source = _prefill_source.replace(_old_fence, _new_fence)
_prefill_namespace = {}
exec(compile(_prefill_source, '<stage-mtp-hidden-capture-lifetime>', 'exec'),
     dsv41.__dict__, _prefill_namespace)
dsv41.DeepseekV41Backbone._forward_layer_major = _prefill_namespace['_forward_layer_major']

signal.alarm(1200)
GIB = 1024**3
RECORD = 18800640
SLOT_BAND_BYTES = 40 * RECORD
CONTROL_ENGINE_BUDGET_BYTES = 88_690_793_288
ENGINE_BUDGET_BYTES = CONTROL_ENGINE_BUDGET_BYTES
ALLOCATOR_LIMIT_BYTES = 98_105_916_352
CONTROL_SLOTS = 80
CONTROL_BASELINE_BYTES = 9_746_600_000
CONTROL_MLX_PEAK_BYTES = 89_622_167_792
CONTROL_PROCESS_PEAK_BYTES = 88_020_030_000
CONTROL_INTERNAL_SYSTEM_PEAK_BYTES = 98_491_498_496
CONTROL_EXTERNAL_SYSTEM_PEAK_BYTES = 99_603_742_720
...
RESERVE_PLANS = {
    ...
    2: (20_166_777_672, 91, 89_686_016),
}
GRAPH_MARGIN = 2 * GIB
...
if stage_depth not in (3, 4, 5):
    raise RuntimeError('bounded depth screen accepts depths 3, 4, or 5 only')
...
engine_ceiling = min(CONTROL_ENGINE_BUDGET_BYTES, ALLOCATOR_LIMIT_BYTES - 6 * GIB)
...
control_path = Path('/tmp/dsv41-110-preflight/python-16k-1024-mtp-compact.jsonl')
control_os_path = Path('/tmp/dsv41-110-preflight/python-16k-1024-mtp-compact.os.jsonl')
control_bounds_path = Path('/tmp/dsv41-110-preflight/python-16k-1024-mtp-compact.bounds.json')
...
if (
    ...
    raise RuntimeError('measured compact cap-80 control evidence changed')
...
if (reference_bounds.get('source_commit') != source
...
try:
    ...
    def cached_ar_logits_row(**kw):
        ...
        expected = {'reference_receipt_sha256': key,
                    'source_commit': source, 'index': index,
