"""Own a read subprocess; reclaim its source pages only after process exit."""
import hashlib
...
files = [Path(installation['model_path']) / 'experts.bin']
...
child = subprocess.Popen([sys.executable, str(ROOT / 'probe.py')])
...
try:
    ...
    print('READ_SOURCE_CACHE_RECLAIMED', json.dumps({k:v for k,v in report.items() if k != 'files'}), flush=True)
    if report['cached_page_bytes_after']:
        raise RuntimeError('read source pages remained cached after child exit')
