...
def resolve_admission(base, wired, *, grow, expected_receipt_hash):
    ...
    spec = importlib.util.spec_from_file_location('native_growth_admission', '/tmp/dsv41-adaptive-depth-20260917/full/native/admission.py')
    ...
    extra = (16 + 64 + 16) * 1024**2  # price all additional plane roots and scheduler state
    ...
    result.update(
        ...
        adaptive_draft_host_allowance_bytes=16 * 1024**2,
        ...
        bound_scope='proved cap84 prefill and native M6/M8 envelope, exact packed storage plus copy allowance and 64MiB for early plane intermediates and 16MiB for shared draft views',
    )
    return result
