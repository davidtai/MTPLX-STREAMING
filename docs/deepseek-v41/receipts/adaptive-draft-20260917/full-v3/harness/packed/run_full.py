...
PACKED_ROOT = Path('/tmp/dsv41-adaptive-depth-20260917/full/packed')
...
GROWTH_ROOT = Path('/tmp/dsv41-adaptive-depth-20260917/full/compat')
...
if stage_depth != 7 or runtime_reserve_gib != 2:
    raise RuntimeError('adaptive draft requires the native D7/M8 envelope and reserve2')
...
if not 0 <= base < 110000000000:
    raise RuntimeError(f'measured baseline {base}B leaves no room below the 110GB ceiling')
...
if decode_steps != 1023 or cache_policy != 'transition-window' or miss_records_per_part != 3 or not shared_overlap:
    raise RuntimeError('adaptive acceptance requires the complete native 16K/1024 workload')
...
try:
    ...
    def checked_load(*a, **kw):
        ...
        from adaptive_lane import install as install_adaptive
        from mlx.utils import tree_flatten
        from mtplx.models import deepseek_v41_dspark_decode as decode_module
        resident.model._dsv41_adaptive_draft = install_adaptive(resident.model, decode_module, tree_flatten=tree_flatten)
    ...
    def run_arm_with_reference(*a, **kw):
        ...
        receipt['adaptive_draft'] = {'policy':'previous-full-acceptance-depth7-else-depth3','initial_depth':5,'maximum_depth':7,'draft_widths':[3,5,7]}
