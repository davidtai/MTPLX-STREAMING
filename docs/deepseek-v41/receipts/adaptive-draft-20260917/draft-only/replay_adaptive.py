...
STAGE=Path('/tmp/dsv41-adaptive-depth-20260917')
...
gc.collect();mx.synchronize();mx.clear_cache()
report = {
    'scope': 'Causal adaptive draft-only replay on saved exact native target states; no target execution, cache-read or throughput claim',
...
for policy in ('fixed5', 'full7_else3', 'ladder357'):
    ...
    while pos < len(ids) - 1:
        ...
        rows.append({'position': pos, 'width': width, 'accepted': accepted,
        ...
        pos += advance
        if policy == 'full7_else3':
            width = 7 if accepted == width else 3
        elif policy == 'ladder357':
            width = 7 if accepted == width else (3 if accepted <= 2 else 5)
    if policy == 'fixed5':
    ...
    row = {'policy': policy, 'cycles': len(rows), 'verify_rows': sum(r['width']+1 for r in rows),
    ...
    print('ADAPTIVE_DRAFT_ARM', json.dumps({k: v for k, v in row.items() if k not in ('rows', 'host_memory')}), flush=True)
