...
const uint slot=pairs[4*assignment];
const uint expert=pairs[4*assignment+1];
const device ushort* wp=(const device ushort*)weights+size_t(slot)*N*(K/4)+outrow*(K/4)+lane*(V/4);
const uint row0=pairs[4*assignment+2];
...
for(uint block=0;block<K/STEP;block++) {{
    ...
    for(uint i=0;i<V;i++) {{
        ...
        x1[i]=row1>=0 ? float(xp1[block*STEP+i]) : 0.0f;
    }}
...
for(int r=0;r<4;r++) {{
    ...
    if(lane==0) {{
        out[row0*N+outrow+r]=T(a);
        if(row1>=0) out[uint(row1)*N+outrow+r]=T(b);
    }}
}}
'''
    return mx.fast.metal_kernel(
        name=f'dsv41_mixed_row_pair_{n}_{k}',
