...
try:
    for sequence,mode in enumerate(proof['arms']):
        ...
        mx.synchronize()
        mx.clear_cache()
    controls = [a['charged_warm_wall_ns'] for a in report['arms'] if a['mode']=='resident-110']
    candidates = [a['charged_warm_wall_ns'] for a in report['arms'] if a['mode']=='expand-111']
