...
assert a['decode_slots_per_layer']==110 and a['physical_bound_bytes']<=110000000000
assert a['embedding_host_allowance_bytes']==32*1024**2
...
(r/'embedding-admission.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps({k:a[k] for k in ('decode_slots_per_layer','physical_bound_bytes',
    ...
    'embedding_host_allowance_bytes','embedding_post_prefill_credit_bytes')}))
