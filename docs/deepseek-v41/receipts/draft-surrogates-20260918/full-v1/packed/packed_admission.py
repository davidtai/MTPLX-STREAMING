...
def resolve_admission(base, wired, *, grow, expected_receipt_hash, strict_allocator):
    ...
    spec = importlib.util.spec_from_file_location('native_growth_admission', '/private/tmp/dsv41-draft-surrogates-20260918/full-v1/native/admission.py')
    ...
    if (original['prefill_active_bound_bytes'] + original['prefill_cache_allowance_bytes'] > allocator_limit
            or original['prefill_physical_bound_bytes'] + embedding_host + lookup_host > DEFAULT_BOX_BUDGET_BYTES):
    ...
    for capacity in range(strict['capacity_search_ceiling'], old_capacity, -1):
        ...
        physical = base + original['host_reserve_bytes'] + embedding_host + lookup_host + active + original['decode_cache_allowance_bytes']
    ...
    result.update(
        ...
        capacity_selection='largest admitted packed capacity above prefill through111; exact weight and resident-scale bytes',
        bound_scope='True144-expert draft subset removes733224960B from resize/seed/steady; original prefill upper bound retained. Adds16MiB fixed-workload lookup metadata within the retained native M8 tensor envelope. Native input table is retired and measured before growth; exact1323827200B credit applies to resize/seed/steady only;32MiB host reserve covers the fixed16MiB row arena plus metadata/copies. Original cap84 prefill envelope retained; exact full tail capture credit applies only during growth; independently measured tail seed bound, with no projection credit; native steady envelope credits packed projection retirement, with no tail credit; original KV/compiler/wired allowances and32MiB helper host reserve retained; only the separately added inactive-cache overshoot allowance is removed under the attested strict allocator; actual256MiB allocator cache applies only after prefill, with64MiB native Engram arenas.',
    )
    return result
