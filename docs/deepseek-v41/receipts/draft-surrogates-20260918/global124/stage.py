...
report['reused_control'] = installation['reused_control']
for mode in ('one_band',):
    ...
    if isinstance(node, ast.Assign) and isinstance(node.value, ast.Call):
        for target in node.targets:
            if isinstance(target, ast.Name) and target.id in ('OUT', 'CURRENT_ROOT'):
                literal_paths[target.id] = Path(ast.literal_eval(node.value.args[0]))
if literal_paths != {'OUT': ROOT/'head-screen.json', 'CURRENT_ROOT': ROOT}:
    raise RuntimeError('cloned screen paths differ from resolved construction root')

proof = dict(prior)
proof.update(source_commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=REPO,text=True).strip(),
    scope='One head-only globally ranked124-expert draft bank, trained on first103 native cycles. Reuse the completed same-source hybrid control. Target states score proposals only; no target execution, physical compaction, or TPS claim.',
    ...
    selection_rule='Global assignment frequency from cycles [0,103), ascending stage/expert ID tie; retain124 total and assert52/46/26 geometry. Removed experts alias nearest retained router vector; target acceptance stays native.',
    physically_compacted=False)
control_path = ROOT.parent/'v2/head-screen.json'
same_source_control = json.loads(control_path.read_text())
assert same_source_control['complete'] and same_source_control['source_commit'] == proof['source_commit']
assert same_source_control['arms'][0]['mode'] == 'hybrid_m8' and same_source_control['arms'][0]['cycles'] == 198
proof['reused_control'] = {'path':str(control_path),'sha256':sha(control_path),
    'mode':'hybrid_m8','cycles':198,'verify_rows':1242,'unchanged_runtime_and_full_physical_draft_bank':True}
