...
def choose(routes, selected):
    ...
    for name, sizes in SIZES.items():
        ...
        ranked = sorted(((s,e,n) for s,c in enumerate(counts) for e,n in c.items()),
                        key=lambda row: (-row[2],row[0],row[1]))[:124]
        global_keep = [sorted(e for stage,e,n in ranked if stage == s) for s in range(3)]
        if keep != global_keep:
            raise RuntimeError('fixed geometry differs from global training frequency selection')
