...
OUT=Path('/private/tmp/dsv41-draft-surrogates-20260918/global124/head-screen.json')
...
CURRENT_ROOT=Path('/private/tmp/dsv41-draft-surrogates-20260918/global124')
...
from pruning import alias_slots
...
for mode in ('one_band',):
    ...
    print('DRAFT_SURROGATE_ARM',json.dumps({k:row[k] for k in ('mode','cycles','verify_rows','verify_width_counts','head_replay_wall_s','mlx_peak_bytes')}),flush=True)
