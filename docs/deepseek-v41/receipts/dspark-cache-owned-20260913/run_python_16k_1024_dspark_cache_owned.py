"""Full workload after projection-cache ownership fix; no peak-saving credit."""
import hashlib
...
signal.alarm(1200)
PREFIX = Path('/tmp/dsv41-110-preflight/python-16k-1024-dspark-cache-owned')
GIB = 1024**3
RECORD = 18800640
FIXED_MTP = 31931661128
MTP_ADDITIONAL = 8353408392
MTP_PEAK = 91544599528
BASE_SLOTS = 67
...
prior = json.loads(Path('docs/deepseek-v41/receipts/dspark-python-16k-20260913/summary.json').read_text())
assert prior['source_commit'] == '4b1e5d369c2d5be35927e09caa3d7d11c3357406'
...
engine = int(110e9 - base - 2 * GIB - 13 * GIB)
...
bounds = {
    ...
    'prior_measured_mtp_peak_bytes': MTP_PEAK, 'prior_slots_per_layer': BASE_SLOTS,
    'prior_source_commit': prior['source_commit'], 'projection_peak_saving_credited_bytes': 0,
    'scope': 'exact native artifact,16K prompt,1023 steps,depth5,pf0,eager target attention,compiled draft,bf16 heads; 13GiB band,2GiB cache,2GiB Python',
}
...
try:
    ...
    if (Path(args.model).resolve() != Path('/Users/davidtai/models/DeepSeek-V4.1-Flash-MTPLX-streaming-mxfp4')
        ...
        or args.max_kv != 17664 or args.box_target_gb != 110 or args.transient_band_gib != 13
