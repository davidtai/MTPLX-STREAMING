...
def run_arm(mode, sequence):
    ...
    try:
        ...
        if reader._fanout_executor._max_workers != 15:
    ...
    print('PACKED_READER_POOL_ARM',json.dumps({k:v for k,v in result.items() if k not in ('cases','reader_metrics','slot_metrics')}),flush=True)
...
try:
    ...
    candidates=[a['warm_total_ns'] for a in report['arms'] if a['mode']=='six-workers']
