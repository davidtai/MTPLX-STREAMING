...
PACKED_ROOT = Path('/tmp/dsv41-router-feature-20260918/full-v2/packed')
...
GROWTH_ROOT = Path('/tmp/dsv41-router-feature-20260918/full-v2/compat')
...
bounds = {
 ...
 'performance_claim':False, 'diagnostic_scope':'first64 native M6 cycles; no prefetch reads;384MiB host plus64MiB Metal reserve',
...
try:
    ...
    def checked_load(*a, **kw):
        global growth_transition, growth_report, prefill_resolved_plan, projection_owner_report, tail_prefill_report, bounded_engram_report
        ...
        def grow_and_install_projection_owners():
            global router_capture
            native_growth()
            projection_owner_report.update(install_model(target_ref(), backbone_type=backbone_type))
            from router_capture import Capture
            router_capture = Capture(target_ref(), PREFIX)
    ...
    def run_arm_with_reference(*a, **kw):
        ...
        receipt['router_capture'] = router_capture.finish()
        ...
        receipt['measurement_origin'] = {
            'ar': 'external_receipt', 'dspark': 'instrumented_diagnostic',
        }
