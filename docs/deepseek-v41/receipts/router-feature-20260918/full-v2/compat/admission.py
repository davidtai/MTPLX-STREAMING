...
def resolve_admission(base, wired, *, grow, expected_receipt_hash):
    ...
    prefill_physical_bound = 108512945408 + max(0, base - 9586573312)
    if (prefill_active_bound + GIB > allocator_limit
        or prefill_physical_bound > 109500000000
        or wired + prefill_active_bound + 2 * GIB > 100 * GIB):
    ...
    if grow:
        ...
        for capacity in range(103, 93, -1):
            ...
            raise RuntimeError('no post-prefill capacity increase fits the measured baseline')
    ...
    physical_bound = max(prefill_physical_bound,
        base + 2 * GIB + max(decode_bound, resize_bound) + cache_allowance)
    return dict(prefill_slots_per_layer=93, decode_slots_per_layer=decode_capacity,
        ...
        host_reserve_bytes=2 * GIB, requested_allocator_cache_limit_bytes=GIB,
