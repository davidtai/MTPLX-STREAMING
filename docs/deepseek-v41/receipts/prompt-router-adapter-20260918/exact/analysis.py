...
def analyze(scores,actual,nrows,persistent,physical,reads):
    ...
    families = {}
    for feature,name in enumerate(('direct','online_bias','online_ridge')):
        sc = scores[feature]
        ...
        for width in (1,2,3,4,6):
            for margin in (0.,.025,.05,.1,.2):
                ...
                for budget in (4,8,12):
                    ...
                    stats = {}
                    for phase,sl in (('train',slice(16,32)),('heldout',slice(32,64))):
    ...
    return {'calibration':'Cycles0..15 fit only the adapters; cycles16..31 choose each layer configuration at >=85% physical-miss precision and >=8 issued records; evaluate unchanged on next32.',
