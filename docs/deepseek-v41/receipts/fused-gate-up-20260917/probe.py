...
from fused_gu import make_dispatch as make_variant
...
report = dict(scope='real layer20 weights, nonidentity physical slots/expert IDs, whole MLP; synthetic BF16 inputs; not full-model performance',
    ...
    kernel_sha256=hashlib.sha256((ROOT / 'fused_gu.py').read_bytes()).hexdigest(), cases=[])


def run():
    global bank, reader, scales
    reader = PositionalExpertReader(model, bypass_page_cache=True, use_native=False, io_read_fanout=4)
    bank = MlxComponentBank(capacity=48, record=records[0], label='fused-gu-proof')
    ...
    for rows, unique, x, bindings, reference in cases:
        ...
        print('FUSED_GU_CASE', json.dumps({key: case[key] for key in ['rows', 'unique_experts', 'parity', 'arm_medians_s', 'latency_reduction_pct']}), flush=True)
...
print('FUSED_GU_COMPLETE', json.dumps({key: report[key] for key in ['mlx_allocator_peak_bytes', 'active_after_close_bytes', 'cleanup']}), flush=True)
