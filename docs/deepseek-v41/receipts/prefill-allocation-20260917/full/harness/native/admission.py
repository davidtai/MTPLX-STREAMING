...
ROOT = Path('/tmp/dsv41-prefill-allocation-20260917/full/native')
...
def resolve_admission(base, wired, *, grow, expected_receipt_hash):
    ...
    capacities = [int(required)] if required is not None else range(100, initial, -1)
    for capacity in capacities:
        if capacity not in range(initial + 1, 101):
