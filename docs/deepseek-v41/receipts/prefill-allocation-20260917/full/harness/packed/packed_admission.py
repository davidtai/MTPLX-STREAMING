...
from prefill_allocation import MAXIMUM_CAPACITY
...
def resolve_admission(base, wired, *, grow, expected_receipt_hash):
    ...
    spec = importlib.util.spec_from_file_location('native_growth_admission', '/tmp/dsv41-prefill-allocation-20260917/full/native/admission.py')
    ...
    extra = (16 + 64 + 16) * 1024**2  # plane roots plus phase-only allocation metadata
    for capacity in range(103, old_capacity, -1):
        weight_after = (40 * capacity + 48) * WEIGHTS
        net_growth = weight_after + PACKED - raw_before
        # The live prompt selects a nonuniform vector only after prefill.
        # Charge its maximum allowed one-component resize before model load.
        packed_copy = (2 * MAXIMUM_CAPACITY - old_capacity) * 5898240
    ...
    result.update(
        ...
        prefill_allocation_metadata_allowance_bytes=16 * 1024**2,
        maximum_layer_decode_capacity=MAXIMUM_CAPACITY,
        ...
        capacity_selection='largest average capacity through103; layer vector selected from prefill counts under max128 and fixed total',
        bound_scope='proved cap84 prefill/native M6-M8 envelope; exact total storage; max128 one-component copy; plane roots and allocation metadata',
    )
    return result
