...
PACKED_ROOT = Path('/tmp/dsv41-prefill-allocation-20260917/full/packed')
...
GROWTH_ROOT = Path('/tmp/dsv41-prefill-allocation-20260917/full/compat')
...
bounds = {
 ...
 'decode_expected_slots_per_layer':None,
 'decode_expected_uniform_equivalent_slots_per_layer':growth_admission['decode_slots_per_layer'],
...
def record_pass(kind, result):
    row = {'pass':kind, 'decode_tok_s':(len(result['generated'])-1)/result['decode_wall_s'],
           ...
           'prefill_slots_per_layer':slots, 'decode_slots_per_layer':None,
           'uniform_equivalent_slots_per_layer':growth_admission['decode_slots_per_layer'],
           'persistent_slots_by_layer':growth_report.get('persistent_slots_by_layer'),
           'growth_payload_bytes':growth_admission['growth_payload_bytes'], 'post_prefill_growth':dict(growth_report), 'runtime_reserve_gib':runtime_reserve_gib,
...
try:
    ...
    def run_arm_with_reference(*a, **kw):
        ...
        reported = receipt['dspark']['serve_stream_counters']['slot_plan']
        if (reported['slots_per_layer'] is not None
            or reported['uniform_equivalent_slots_per_layer'] != growth_admission['decode_slots_per_layer']
            or reported['persistent_slots_by_layer'] != growth_report['persistent_slots_by_layer']):
