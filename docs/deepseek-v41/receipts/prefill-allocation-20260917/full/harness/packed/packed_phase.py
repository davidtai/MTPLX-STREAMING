...
from prefill_allocation import capacities_from_prefill, MAXIMUM_CAPACITY
...
def install_growth(model, capacity, *, mx, admission):
    ...
    if (layers != tuple(range(40)) or rt.spec.expert_count != 384 or rt.spec.top_k != 6
        ...
        or not old_capacity < capacity <= 103
        or admission['maximum_layer_decode_capacity'] != MAXIMUM_CAPACITY
    ...
    def transition():
        ...
        with ExitStack() as stack:
            ...
            capacities = capacities_from_prefill(
                {layer: rt._banks[layer]._prefill_route_freq for layer in layers}, capacity)
            if (sum(capacities) != capacity * len(layers)
                or min(capacities) < old_capacity or max(capacities) > MAXIMUM_CAPACITY):
                raise RuntimeError('prefill allocation exceeds the admitted vector envelope')
            new_plan = replace(new_plan, persistent_slots_by_layer=tuple(zip(layers, capacities)))
            new_config = replace(new_config, persistent_slots_by_layer=capacities)
            fixed_allocator.plan = new_plan
            state.update(decode_slots_per_layer=None, uniform_equivalent_slots_per_layer=capacity,
                         persistent_slots_by_layer={str(layer): capacities[layer] for layer in layers},
                         allocation_policy='prefill_frequency_marginal; fixed for complete decode')
            ...
            try:
                ...
                for layer, switch in zip(layers, switches):
                    ...
                    if capacities[layer] > old_capacity:
                        grow_bank(bank, capacities[layer], mx=mx)
                ...
                for layer in layers:
                    bank = allocator.banks['persistent', layer]
                    for row in range(old_capacity, capacities[layer]):
                ...
                for layer, policy in rt._banks.items():
                    layer_capacity = capacities[layer]
                    policy._slot_to_expert.extend([None] * (layer_capacity - old_capacity))
                    policy.persistent_slots = policy._persistent_capacity = layer_capacity
                    policy.slot_count = layer_capacity + policy.transient_slots + policy.prefetch_slots
                    policy._protected_cap = max(1, int(layer_capacity * 0.8))
                ...
                pool._persistent_route_capacity = None
                pool._persistent_route_capacities = dict(zip(layers, capacities))
