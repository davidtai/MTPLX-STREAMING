...
PACKED_ROOT = Path('/private/tmp/dsv41-extension-draft-20260919/full-v1/packed')
...
# Draft-only approximate aliases, fixed from a completed head quality screen.
...
_COMPACT_RESIDENT_ROOT = Path('/private/tmp/dsv41-draft-surrogates-20260918/artifact')
...
if (MTP_PRUNED_EXPERTS, MTP_PRUNED_BYTES) != (240, 4_512_153_600):
...
GROWTH_ROOT = Path('/private/tmp/dsv41-extension-draft-20260919/full-v1/compat')
...
RESERVE_PLANS = {
    ...
    2: (19_433_552_712, 94, 89_686_016),
}
GRAPH_MARGIN = 2 * GIB
...
if engine != 89461619528 + (slots - 93) * SLOT_BAND_BYTES:
...
try:
    ...
    def admitted_mtp(*a, **kw):
        ...
        sample()
        rt = kw['model']._mtplx_expert_runtime
        with rt.admit_kv_tokens(len(kw['prompt_ids']) + int(kw['steps']) + 7 + 1):
            try:
                try:
