...
def resolve_admission(base, wired, *, grow, expected_receipt_hash, strict_allocator):
    ...
    spec = importlib.util.spec_from_file_location('native_growth_admission', '/private/tmp/dsv41-extension-draft-20260919/full-v1/native/admission.py')
    ...
    draft_proof = installation['draft_surrogates']
    ...
    allocator_limit = original['allocator_limit_bytes'] - embedding_host - lookup_host - expansion_host - background_growth
    if (original['prefill_active_bound_bytes'] + original['prefill_cache_allowance_bytes'] > allocator_limit
            or original['prefill_physical_bound_bytes'] + embedding_host + lookup_host + expansion_host + background_growth > DEFAULT_BOX_BUDGET_BYTES):
        raise RuntimeError('input-row host allowance does not fit unchanged prefill')
    for capacity in range(113, old_capacity, -1):
        ...
        steady -= embedding_credit + draft_credit
        resize -= embedding_credit + draft_credit
        seed -= embedding_credit + draft_credit
        ...
        physical = base + original['host_reserve_bytes'] + embedding_host + lookup_host + expansion_host + background_growth + active + original['decode_cache_allowance_bytes']
    ...
    result.update(
        ...
        host_reserve_bytes=original['host_reserve_bytes'] + embedding_host + lookup_host + expansion_host + background_growth,
        prefill_physical_bound_bytes=original['prefill_physical_bound_bytes'] + embedding_host + lookup_host + expansion_host + background_growth,
        ...
        capacity_search_ceiling=113,
        existing_bank_rows=84, maximum_extension_bank_rows=29,
        ...
        physical_bound_bytes=max(original['prefill_physical_bound_bytes'] + embedding_host + lookup_host + expansion_host + background_growth, physical),
        packed_inventory_manifest_sha256=proof['artifact_manifest_sha256'],
        native_capacity_selection=original['capacity_selection'],
        capacity_selection='largest admitted85..113 final slots;84 original rows plus1..29 extension rows after native seed, no expert-bank copies',
        bound_scope='Native144-expert draft removes733224960B from seed/transition/steady, no prefill credit. Explicit256MiB background allowance prices the prior107409300B launch-estimate miss with additional headroom. Unchanged84-row prefill. First transition retains84 expert rows, retires raw scales and installs packed scales; price the entire packed inventory plus one packed layer above transition-start instead of crediting its negative final delta. Native MTP seed completes at84 rows. Extension peak includes all final added packed rows, one full extension-bank raw-scale temporary, three BF16 projection arrays and page padding. Existing256MiB allocation margin, nativeM8/KV envelope, 256MiB strict allocator cache,100GiB wired ceiling and110GB whole-machine ceiling retained. Packed projection credit only in steady state. Extra16MiB schedule/bank metadata reserve remains in every phase. No existing expert backing array is resized or copied.',
    )
    return result
