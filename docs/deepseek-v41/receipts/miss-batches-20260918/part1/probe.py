"""One-layer native route replay for miss-completion batch size."""
import dataclasses
import gc
import hashlib
import json
...
import plane_lane
import plane_lane_part2
import plane_lane_part1
...
report = {'source_commit':proof['source_commit'], 'scope':proof['scope'], 'construction':proof,
          'before':before, 'arms':[], 'complete':False}
references = {}
scales = None


def run_arm(part_size, sequence):
    reader = allocator = slots = runtime = switch = runners = None
    x = shared_input = indices = y = shared = None
    result = {'part_size':part_size, 'sequence':sequence, 'cases':[]}
    try:
        ...
        lane = plane_lane_part2 if part_size==2 else plane_lane_part1
    ...
    print('MISS_PART_ARM',json.dumps({k:v for k,v in result.items() if k not in ('cases','reader_metrics','slot_metrics')}),flush=True)
...
try:
    ...
    for sequence,part_size in enumerate(proof['arms']):
        with expert_routing_phase('decode'):
            run_arm(part_size,sequence)
    controls=[a['warm_total_ns'] for a in report['arms'] if a['part_size']==2]
    candidates=[a['warm_total_ns'] for a in report['arms'] if a['part_size']==1]
...
print('MISS_PART_COMPLETE',json.dumps({k:report[k] for k in ('complete','latency_ratio','control_spread_fraction','active_after_close_bytes')}),flush=True)
