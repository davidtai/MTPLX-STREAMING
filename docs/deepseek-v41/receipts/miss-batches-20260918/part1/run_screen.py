...
files += [ROOT / 'artifact' / component[field]['file']
          for component in inventory['layers'][installation['layer']]['components'].values()
...
try:
    ...
    print('PREFIX_MLP_SOURCE_CACHE_RECLAIMED', json.dumps({k:v for k,v in report.items() if k != 'files'}), flush=True)
