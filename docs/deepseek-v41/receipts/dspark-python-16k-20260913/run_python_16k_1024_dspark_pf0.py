"""First current-source MTP full workload with a phase-bounded extra reserve."""
import hashlib
...
signal.alarm(1200)
PREFIX = Path('/tmp/dsv41-110-preflight/python-16k-1024-dspark-pf0')
GIB = 1024**3
RECORD = 18800640
...
draft = json.loads(Path('/tmp/dsv41-110-preflight/dspark-head-peak.json').read_text())
assert draft['ok'] and draft['eager_compiled_draft_ids_equal']
assert draft['parameter_bytes'] == 10597621640
seed = next(p for p in draft['phases'] if p['phase'] == 'prefill_seed_16k')
seed_extra = seed['peak_bytes'] - draft['seed_baseline_active_bytes']
draft_extra = max(draft['draft_eager']['peak_delta_bytes'], draft['draft_compiled']['peak_delta_bytes'])
# Added target-prefill payload: three copies of the 16K x (3*5120) fp32 main
# hidden (per-target means, per-chunk concatenations, final concatenation).
# The same layer-major target/HС buffers already appear in the AR peak.
# Verify uses <=6 rows, per-layer routing/gather fences and the same fixed
# 48-record transient pool; 2 GiB bounds additional selected attention operands,
# full-source L20 index scores and cache rewinds. The prior real-shape one-layer
# attention census peaked at1.108 GB INCLUDING weights and four queued calls.
# Seed, draft, verify and target prefill are sequential phases. Keep 1 GiB beyond
# their maximum additional payload for graph/workspace variation, including the
# prior AR storage-delta projection's5.2 MB shortfall.
phase_extra = {
    'target_prefill_main_hidden_bytes': 3 * 16384 * (3 * 5120) * 4,
    'draft_window_seed_bytes': seed_extra,
    'draft_bytes': draft_extra,
    'target_verify_bytes': 2 * GIB,
}
extra_bound = max(phase_extra.values()) + GIB
assert extra_bound < 4 * GIB
engine = int(110e9 - base - 2 * GIB - 14 * GIB)
slots = (engine - FIXED_MTP) // (40 * RECORD)
assert 1 <= slots <= 72
active_bound = AR_PEAK + (slots - BASE_SLOTS) * 40 * RECORD + MTP_ADDITIONAL + extra_bound
...
bounds = {
    ...
    'wired_before_bytes': wired, 'phase_additional_bounds': phase_extra,
    'graph_workspace_margin_bytes': GIB, 'extra_active_bound_bytes': extra_bound,
    'draft_probe_source': draft['source_commit'],
    'scope': 'exact native artifact,16K prompt,1023 steps,depth5,pf0,eager target attention,compiled draft,bf16 heads; 14GiB band,2GiB cache,2GiB Python',
}
...
try:
    ...
    if (Path(args.model).resolve() != Path('/Users/davidtai/models/DeepSeek-V4.1-Flash-MTPLX-streaming-mxfp4')
        ...
        or args.max_kv != 17664 or args.box_target_gb != 110 or args.transient_band_gib != 14
