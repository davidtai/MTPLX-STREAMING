...
def resolve_admission(base, wired, *, grow, expected_receipt_hash):
    ...
    capacities = [int(required)] if required is not None else range(100, 83, -1)
    for capacity in capacities:
        if capacity not in range(84, 101):
            raise RuntimeError('explicit matched decode capacity must be84..100')
    ...
    return dict(physical_ceiling_bytes=DEFAULT_BOX_BUDGET_BYTES, prefill_slots_per_layer=initial, decode_slots_per_layer=capacity,
        ...
        capacity_selection='largest admitted84..100; explicit fixed value required for a fresh matched control')
