...
os.environ['MTPLX_DSV41_DSPARK_CONF_THRESHOLD'] = str(CONFIDENCE_THRESHOLD)
...
GROWTH_ROOT = Path('/tmp/dsv41-confidence-20260917/growth84')
...
if stage_depth not in (5, 7) or runtime_reserve_gib != 2:
    raise RuntimeError('native width comparison requires depth5 or7/reserve2')
# Bind the actual block and all child stages once before construction.
if stage_depth == 7:
    _native_head_init = dspark.DSparkHead.__init__
    def _head7_init(self, args):
        if args.dspark_block_size != 5:
            raise RuntimeError('native artifact block declaration changed')
        _native_head_init(self, replace(args, dspark_block_size=7))
    dspark.DSparkHead.__init__ = _head7_init
...
from admission import resolve_admission
if decode_steps != 1023 or cache_policy != 'transition-window' or miss_records_per_part != 3 or not shared_overlap:
    raise RuntimeError('width comparison requires the complete native16K/1024 workload')
...
if slots != 84 or engine != 83426614088:
...
bounds = {
 ...
 'confidence_threshold':CONFIDENCE_THRESHOLD, 'post_prefill_growth':GROWTH_ENABLED, 'growth_admission':growth_admission,
...
def record_pass(kind, result):
    row = {'pass':kind, 'confidence_threshold':CONFIDENCE_THRESHOLD, 'decode_tok_s':(len(result['generated'])-1)/result['decode_wall_s'],
...
try:
    ...
    def checked_load(*a, **kw):
        ...
        if (resident.model.mtp.block_size != stage_depth
            or any(s.block_size != stage_depth for s in resident.model.mtp.layers)):
            raise RuntimeError('actual native draft block width differs from the request')
    ...
    def run_arm_with_reference(*a, **kw):
        ...
        if receipt['dspark']['depth'] != stage_depth or receipt['dspark']['verify_chunks'] != [stage_depth + 1]:
            raise RuntimeError('native verify did not execute the installed width')
        if growth_report.get('phase') != 'decode':
        ...
        receipt['dspark']['confidence_threshold'] = CONFIDENCE_THRESHOLD
        receipt['arm_env']['MTPLX_DSV41_DSPARK_CONF_THRESHOLD'] = str(CONFIDENCE_THRESHOLD)
        receipt['actual_draft_block_size'] = stage_depth
        receipt['artifact_draft_block_size'] = 5
