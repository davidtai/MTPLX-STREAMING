...
OUT=Path('/tmp/dsv41-confidence-20260917/screen.json')
...
if subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip()!='7953020cda67ccda296a729d01a03adb9cd1ae4b':
    raise RuntimeError('source commit differs from the teacher')
for path,expected in installation['runtime_source_sha256'].items():
    if path == 'scripts/deepseek_v41/ab_decode_env_levers.py':
        continue  # Reporting-only edits; this runner is never imported here.
...
gc.collect();mx.synchronize();mx.clear_cache()
from mtplx.models.deepseek_v41_dspark_decode import _effective_draft_len
report = {'scope': 'Native D5 confidence trimming on saved exact M6 target states. Futures score proposals but do not select widths. No target execution or throughput claim.',
...
for threshold in (None, 0.25, 0.5, 0.75, 0.9):
    ...
    while pos < len(ids)-1:
        ...
        keep=_effective_draft_len(conf,5,threshold)
        ...
        rows.append({'position':pos,'effective_draft_length':keep,'confidence':confidence.tolist(),
                     'proposed_ids':[int(v) for v in proposed], 'teacher_matched_prefix':native_accepted,
    ...
    if threshold is None:
        ...
        if commit_lengths!=expected:raise RuntimeError('native D5 control does not reproduce captured boundaries')
    row={'threshold':threshold,'cycles':len(commit_lengths),
    ...
    print('CONFIDENCE_SCREEN_ARM',json.dumps({k:row[k] for k in ('threshold','cycles','verify_rows','verify_width_counts','head_replay_wall_s','mlx_peak_bytes')}),flush=True)
