...
ROOT = Path('/tmp/dsv41-explicit-profile-20260917/growth84')
...
def resolve_admission(base, wired, *, grow, expected_receipt_hash):
    ...
    capacities = [int(required)] if required is not None else range(100, 93, -1)
    for capacity in capacities:
        if capacity not in range(94, 101):
            raise RuntimeError('explicit matched decode capacity must be94..100')
    ...
    return dict(prefill_slots_per_layer=initial, decode_slots_per_layer=capacity,
        ...
        capacity_selection='largest admitted94..100; explicit fixed value required for a fresh matched control')
