...
if subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip()!=teacher['source_commit']:
    raise RuntimeError('source commit differs from the teacher')
for path,expected in installation['runtime_source_sha256'].items():
    if hashlib.sha256(Path(path).read_bytes()).hexdigest()!=expected:
        raise RuntimeError('source bytes differ from the teacher')
...
if (snap['box']['used_bytes']+ACTIVE_BOUND+CACHE_ALLOWANCE+HOST_ALLOWANCE>109500000000
...
gc.collect();mx.synchronize();mx.clear_cache()
report={'scope':'Draft-only oracle acceptance on fixed exact M6 target states; not target execution or decode-TPS evidence',
        'teacher_sha256':digest_file(TEACHER_PATH),'reference_receipt_sha256':digest_file(reference_path),
        'source_commit':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),
        'script_sha256':digest_file(Path(__file__)),
        'runtime_source_sha256':installation['runtime_source_sha256'],
        'config_sha256':digest_file(ARTIFACT/'config.json'),
        'artifact_block_size':5,'static_active_bound_bytes':ACTIVE_BOUND,'cache_allowance_bytes':CACHE_ALLOWANCE,
        'host_allowance_bytes':HOST_ALLOWANCE,'allocator_limit_bytes':48*GIB,'baseline':snap,'arms':[]}
ids=teacher['token_ids']
for mode in ('compact','full'):
    for width in (5,7,9,13):
        owner=DraftOnly(width,compact=(mode=='compact'))
        current=dict(tree_flatten(owner.parameters()))
        if set(current)!=set(weights[mode]):
            raise RuntimeError(f'draft parameter coverage mismatch missing={sorted(set(current)-set(weights[mode]))[:12]} unexpected={sorted(set(weights[mode])-set(current))[:12]}')
        del current
        owner.load_weights(list(weights[mode].items()),strict=True)
        if any(value is not weights[mode][name] for name,value in tree_flatten(owner.parameters())):
            raise RuntimeError('initial random parameters were not completely replaced')
        ...
        pos=0;main_h=initial_main;commit_lengths=[];accepted_by_depth=[0]*width;drafted_by_depth=[0]*width
        ...
        while pos<len(ids)-1:
            ...
            accepted=0
            for depth in range(min(width,len(ids)-1-pos)):
                drafted_by_depth[depth]+=1
                if int(proposed[depth])!=ids[pos+depth+1]: break
                accepted_by_depth[depth]+=1;accepted+=1
        ...
        control_ok=None
        if mode=='compact' and width==5:
            expected=list(teacher['commit_lengths']);expected[-1]=1023-sum(expected[:-1])
            control_ok=(commit_lengths==expected)
            if not control_ok:
                mismatch=next((i for i,(a,b) in enumerate(zip(commit_lengths,expected)) if a!=b),min(len(commit_lengths),len(expected)))
                failure={'reason':'D5 replay does not reproduce exact captured commit boundaries','first_cycle':mismatch,'replay_cycles':len(commit_lengths),'teacher_cycles':len(expected),'replay_prefix':commit_lengths[:mismatch+2],'teacher_prefix':expected[:mismatch+2]}
                (ROOT/'draft-replay-rejected.json').write_text(json.dumps(failure,indent=2)+'\n')
                raise RuntimeError(f'D5 replay is not faithful at cycle {mismatch}')
        row={'resident_mode':mode,'block_size':width,'cycles':len(commit_lengths),
             'mean_committed_tokens':1023/len(commit_lengths),'accepted_by_depth':accepted_by_depth,
             'drafted_by_depth':drafted_by_depth,'commit_lengths':commit_lengths,
             'head_replay_wall_s':wall,'exact_control_commit_boundaries':control_ok,
        ...
        print('DRAFT_SCREEN_ARM',json.dumps({k:row[k] for k in ('resident_mode','block_size','cycles','mean_committed_tokens','head_replay_wall_s','exact_control_commit_boundaries','mlx_peak_bytes')}),flush=True)
        del owner,caches,main_h,out,logits,conf
        gc.collect();mx.synchronize();mx.clear_cache()
