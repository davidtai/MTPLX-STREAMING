...
GROWTH_ROOT = Path('/tmp/dsv41-depth-replay-20260917')
...
bounds = {
 ...
 'source_compatibility':COMPATIBILITY, 'diagnostic_only':True, 'performance_eligible':False,
...
try:
    ...
    if (Path(args.model).resolve() != Path('/Users/davidtai/models/DeepSeek-V4.1-Flash-MTPLX-streaming-mxfp4')
        ...
        or args.allocator_cache_gib != 1 or args.host_overhead_gib != 2.25
    ...
    from teacher_capture import TeacherCapture, validate_storage_bridge
    validate_storage_bridge()
    ...
    def checked_load(*a, **kw):
        global growth_transition, growth_report, prefill_resolved_plan, teacher
        ...
        teacher = TeacherCapture(resident.model, GROWTH_ROOT, steps=decode_steps)
        if GROWTH_ENABLED:
    ...
    def observe_seed_prefill(*a, **kw):
        ...
        teacher.capture_prefill(result, a[2] if len(a)>2 else kw['mtp_caches'])
    ...
    def run_arm_with_reference(*a, **kw):
        ...
        receipt['teacher_trace'] = teacher.finalize(receipt['dspark']['token_ids'],
            cycles=receipt['dspark']['cycles'], prompt_sha256=receipt['prompt_ids_sha256'], source_commit=source)
        receipt['diagnostic_only'] = receipt['dspark']['diagnostic_only'] = True
        receipt['performance_eligible'] = receipt['dspark']['performance_eligible'] = False
