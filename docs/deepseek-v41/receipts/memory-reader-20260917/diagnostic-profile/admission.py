...
ROOT = Path('/tmp/dsv41-depth7-full-20260917')
...
def resolve_admission(base, wired, *, grow, expected_receipt_hash):
    ...
    prefill_physical = base + 2 * GIB + prefill_active + prefill_cache
    if (prefill_physical > 109500000000
        or prefill_active + prefill_cache > allocator_limit
        or wired + prefill_active + prefill_cache + GIB > 100 * GIB):
        raise RuntimeError(f'cap91 prefill cannot fit the current baseline: {prefill_physical}B')
    ...
    capacities = [int(required)] if required is not None else range(100, 95, -1)
    for capacity in capacities:
        if capacity not in range(96, 101):
            raise RuntimeError('explicit matched decode capacity must be96..100')
        ...
        physical = base + 2 * GIB + active + cache_allowance
        if (physical <= 109500000000 and active + cache_allowance <= allocator_limit
    ...
    return dict(prefill_slots_per_layer=initial, decode_slots_per_layer=capacity,
        ...
        prefill_cache_allowance_bytes=prefill_cache, host_reserve_bytes=2 * GIB,
        ...
        capacity_selection='largest admitted96..100; explicit fixed value required for a fresh matched control')
