...
PREFIX = Path("/tmp/dsv41-110-preflight/mtp-verify-routes-16k-1024-v2")
...
prior_72 = json.loads(Path(
    "docs/deepseek-v41/receipts/dspark-layer-prefill-20260913/summary.json"
).read_text())
exact_six_slot_bytes = 6 * 40 * RECORD
observed_six_slot_active_delta = (
    PRIOR_ACTIVE_PEAK - round(prior_72["dspark"]["peak_gb"] * 1e9)
)
if abs(exact_six_slot_bytes - observed_six_slot_active_delta) > 1_000_000:
    raise RuntimeError("measured 72-to-78-slot active delta no longer matches storage")
...
# No KV or prefill saving is credited. A smaller plan physically omits complete
# bank images: the observed 72->78 active delta agrees with the exact slot
# storage delta to 16,276 B. Diagnostic timing is void.
engine = int(110e9 - base - 2 * GIB - 7 * GIB)
expected_slots = (engine - FIXED_MTP) // (40 * RECORD)
if not 64 <= expected_slots <= 78:
    raise RuntimeError("unadmitted full-workload slot geometry")
slot_delta = (expected_slots - PRIOR_SLOTS) * 40 * RECORD
...
bounds = {
    ...
    "signed_exact_slot_storage_delta_bytes": slot_delta,
    "observed_72_to_78_active_delta_bytes": observed_six_slot_active_delta,
...
try:
    ...
    if (Path(args.model).resolve() != ROOT or
        ...
        args.box_target_gb != 110 or args.transient_band_gib != 7 or
