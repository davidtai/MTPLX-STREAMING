"""Own a draft subprocess; reclaim its source pages only after process exit."""
import hashlib
...
for name in ('draft-replay.json', 'child.json', 'reclamation.json'):
    if (ROOT / name).exists():
...
files = reclaim.model_files(Path(installation['model_path']))
...
child = subprocess.Popen([sys.executable, str(ROOT / 'replay_even.py')])
...
try:
    ...
    print('DRAFT_SOURCE_CACHE_RECLAIMED', json.dumps({k:v for k,v in report.items() if k != 'files'}), flush=True)
