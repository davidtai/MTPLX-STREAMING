...
def resolve_admission(base, wired, *, grow, expected_receipt_hash):
    ...
    # The separately proved native M8 envelope includes cap91 prefill and bounds
    # both native M6 and M8. Retain its extra KV, padding and compiler margins.
    spec = importlib.util.spec_from_file_location('native_growth_admission', '/tmp/dsv41-depth7-full-20260917/admission.py')
    ...
    result.update(
        ...
        bound_scope='proved cap91 prefill and native M6/M8 envelope, changing only exact expert storage bytes plus packed-copy allowance',
    )
    return result
