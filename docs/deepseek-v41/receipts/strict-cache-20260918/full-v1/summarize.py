...
PREFIX=Path('/tmp/dsv41-110-stage/full-strict-cache-20260918-v1')
...
result={
    ...
    'performance_scope':'One complete strict-cache allocator candidate versus retained part3 memory-compose result; same exact workload and native arithmetic, but different baseline and capacity. This is not a matched-capacity or repeatability proof.',
    'goal_20_tps_reached':d['decode_tok_s']>=20,
}
result.update(
    cache_limit_phase=d['memory_cap']['cache_limit_phase'],
    allocator_cache_phase_limits_bytes=d['memory_cap']['allocator_cache_phase_limits_bytes'],
    reported_current_cache_limit_bytes=d['memory_cap']['cache_limit_bytes'],
    host_reserve_bytes=bound['growth_admission']['host_reserve_bytes'],
    decode_cache_allowance_bytes=bound['growth_admission']['decode_cache_allowance_bytes'],
    decode_cache_overshoot_allowance_bytes=bound['growth_admission']['decode_cache_overshoot_allowance_bytes'],
    bounded_engram=receipt['bounded_engram'],
)
assert result['reported_current_cache_limit_bytes']==256*1024**2
assert result['decode_cache_overshoot_allowance_bytes']==0
assert bound['growth_admission']['strict_cache_overshoot_credit_bytes']==2258155644
result['strict_allocator']=receipt['strict_allocator']
assert result['host_reserve_bytes']==1371664384
assert all(b['budget_bytes']==67108864 for b in result['bounded_engram']['banks'])
assert result['input_tokens']==16384 and result['output_tokens']==1024
assert result['decode_miss_records_per_part']==3
