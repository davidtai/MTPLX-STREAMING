...
report = {'source_commit':proof['source_commit'], 'scope':proof['scope'], 'construction':proof,
          'before':before, 'binary':BINARY, 'library':library, 'arms':[], 'complete':False}
references = {}
scales = None


def run_arm(cache_bytes, sequence):
    part_size = 3
    mx.synchronize()
    mx.clear_cache()
    ...
    result = {'part_size':part_size, 'cache_limit_bytes':cache_bytes, 'sequence':sequence, 'cases':[]}
    try:
    ...
    print('ALLOCATOR_CACHE_ARM',json.dumps({k:v for k,v in result.items() if k not in ('cases','reader_metrics','slot_metrics')}),flush=True)
...
try:
    ...
    for sequence,cache_bytes in enumerate(proof['arms']):
        with expert_routing_phase('decode'):
            run_arm(cache_bytes,sequence)
...
print('ALLOCATOR_CACHE_COMPLETE',json.dumps({k:report[k] for k in ('complete','binary','active_after_close_bytes')}),flush=True)
