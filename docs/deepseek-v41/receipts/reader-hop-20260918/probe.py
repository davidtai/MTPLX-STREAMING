...
def run_arm(mode, sequence):
    ...
    print('READER_HOP_ARM',json.dumps({k:v for k,v in result.items() if k not in ('cases','reader_metrics','slot_metrics')}),flush=True)
...
try:
    ...
    candidates=[a['warm_total_ns'] for a in report['arms'] if a['mode']=='miss-worker']
