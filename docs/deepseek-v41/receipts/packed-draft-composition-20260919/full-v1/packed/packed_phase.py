...
def install_growth(model, capacity, *, mx, admission):
    ...
    def transition():
        ...
        with ExitStack() as stack:
            ...
            cap_report = rt.memory_cap_report
            if not isinstance(cap_report, dict) or cap_report.get('cache_limit_bytes') != 1024**3:
                raise RuntimeError('prefill allocator-cache policy differs')
            previous_cache_limit = int(mx.set_cache_limit(256 * 1024**2))
            if previous_cache_limit != 1024**3:
                raise RuntimeError('live prefill allocator-cache limit differs')
            mx.clear_cache()
            state['allocator_cache_phase_limits_bytes'] = {'prefill': 1024**3, 'growth_decode': 256 * 1024**2}
            cap_report.update(cache_limit_bytes=256 * 1024**2,
                allocator_cache_limit_bytes=256 * 1024**2, allocator_cache_limit_gib=0.25,
                previous_cache_limit_bytes=previous_cache_limit,
                cache_limit_phase='growth_decode', slot_derivation_phase='initial_loaded_prefill',
                allocator_cache_phase_limits_bytes=dict(state['allocator_cache_phase_limits_bytes']))
