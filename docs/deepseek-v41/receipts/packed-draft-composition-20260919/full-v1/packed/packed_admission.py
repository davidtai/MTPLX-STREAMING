...
def resolve_admission(base, wired, *, grow, expected_receipt_hash, strict_allocator):
    ...
    spec = importlib.util.spec_from_file_location('native_growth_admission', '/private/tmp/dsv41-packed-draft-full-20260919/full-v1/native/admission.py')
    ...
    allocator_limit = original['allocator_limit_bytes'] - embedding_host - lookup_host - expansion_host - draft_host - background_growth
    if (original['prefill_active_bound_bytes'] + original['prefill_cache_allowance_bytes'] > allocator_limit
            or original['prefill_physical_bound_bytes'] + embedding_host + lookup_host + expansion_host + draft_host + background_growth > DEFAULT_BOX_BUDGET_BYTES):
    ...
    for capacity in range(112, old_capacity, -1):
        ...
        append_peak = seed + overflow_payload + (capacity-old_capacity)*(RAW-WEIGHTS) + 67108864 + original['page_padding_allowance_bytes']
        active = max(steady, resize, seed, early_prime_peak, append_peak)
        physical = base + original['host_reserve_bytes'] + embedding_host + lookup_host + expansion_host + draft_host + background_growth + active + original['decode_cache_allowance_bytes']
    ...
    result.update(
        ...
        host_reserve_bytes=original['host_reserve_bytes'] + embedding_host + lookup_host + expansion_host + draft_host + background_growth,
        prefill_physical_bound_bytes=original['prefill_physical_bound_bytes'] + embedding_host + lookup_host + expansion_host + draft_host + background_growth,
        ...
        physical_bound_bytes=max(original['prefill_physical_bound_bytes'] + embedding_host + lookup_host + expansion_host + draft_host + background_growth, physical),
        ...
        bound_scope='Unchanged84-row prefill. First transition retains84 expert rows, retires raw scales and installs packed scales; price the entire packed inventory plus one packed layer above transition-start instead of crediting its negative final delta. Native MTP seed completes at84 rows. Cold projection prime occurs at84 rows and prices three BF16 arrays. Extension owns all final added rows, one raw-scale temporary, one already initialized BF16 projection and page padding. Draft-only403MB credit applies to steady state;128MiB new GPU workspace,16MiB host metadata and256MiB background allowance are explicit. Native93/58/32 draft experts remain. Existing256MiB allocation margin, nativeM8/KV envelope, 256MiB strict allocator cache,100GiB wired ceiling and110GB whole-machine ceiling retained. Packed projection credit only in steady state. Extra16MiB schedule/bank metadata reserve remains in every phase. No existing expert backing array is resized or copied.',
    )
    return result
