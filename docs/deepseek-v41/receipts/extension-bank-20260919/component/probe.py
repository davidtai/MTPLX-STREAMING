...
def run_arm(mode, sequence):
    ...
    print('PREDICTABLE_EXPANSION_ARM',json.dumps({k:v for k,v in result.items() if k!='output_digests'}),flush=True)
...
try:
    for sequence,mode in enumerate(proof['arms']):
        ...
        mx.synchronize()
        mx.clear_cache()
    controls = [a['charged_warm_wall_ns'] for a in report['arms'] if a['mode']=='resize109-plus1']
    candidates = [a['charged_warm_wall_ns'] for a in report['arms'] if a['mode']=='extension84-plus26']
...
print('PREDICTABLE_EXPANSION_COMPLETE',json.dumps({k:v for k,v in report.items() if k not in ('construction','arms')}),flush=True)
