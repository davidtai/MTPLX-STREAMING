...
ROOT = Path('/tmp/dsv41-memory-compose-20260918/full-v1/native')
...
def resolve_admission(base, wired, *, grow, expected_receipt_hash):
    ...
    if os.environ.get('MTPLX_ENGRAM_CACHE_LIMIT') != '119537664':
        raise RuntimeError('explicit bounded Engram cache required')
    host_reserve = python_cache_budget(os.environ)['required_host_bytes'] + 32 * 1024**2
    if host_reserve != 1578202112:
        raise RuntimeError('bounded host inventory changed')
    ...
    return dict(physical_ceiling_bytes=DEFAULT_BOX_BUDGET_BYTES, prefill_slots_per_layer=initial, decode_slots_per_layer=capacity,
        ...
        prefill_cache_allowance_bytes=prefill_cache, host_reserve_bytes=host_reserve, projection_host_allowance_bytes=16 * 1024**2, tail_host_allowance_bytes=16 * 1024**2, engram_bytes_per_bank=119537664,
