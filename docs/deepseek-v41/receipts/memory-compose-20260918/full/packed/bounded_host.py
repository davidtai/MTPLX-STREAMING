...
def describe_engram(model):
    ...
    if budget['required_host_bytes'] != 1544647680 or len(model._engram_banks) != 2:
        raise RuntimeError('bounded Engram host inventory differs')
    banks = []
    for bank in model._engram_banks:
        cache = bank.cache
        if (cache.budget_bytes != 119537664 or cache.row_bytes < 264
            or cache.slot_count != 119537664 // cache.row_bytes
            or cache._arena.shape != (cache.slot_count, cache.row_bytes)
            or cache._arena.dtype.name != 'uint8'
            or cache._arena.nbytes > 119537664):
