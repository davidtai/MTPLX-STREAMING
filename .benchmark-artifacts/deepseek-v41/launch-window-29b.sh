...
OUT=$WT/docs/deepseek-v41/receipts/gpu-windows/window-29b
mkdir -p "$OUT"
cd "$WT"
for i in $(seq 1 90); do curl -s -m 3 http://127.0.0.1:8080/health | grep -q '"ok":true' && break; sleep 10; done
echo "[launcher] $(date -u +%FT%TZ) agent healthy; starting window 29b"
...
F='transformers\]|█|╗|╝|║|╭|╰|│'
echo '[step 1] AR prefill_lean_sel @16384 (control: window-28b arm), decode 256, stage timing, 80 GiB'; MTPLX_ROUTE_STAGE_PROBE=1 \$PY scripts/deepseek_v41/ab_decode_env_levers.py --context-tokens 16384 --decode-tokens 256 --max-kv 17408 --arms prefill_lean_sel --stage-timing --memory-limit-gib 80 --out $OUT/ar-16k-control.json 2>&1 | grep -vE \"\$F\" | tail -14
echo '[step 2] AR prefill_lean_sel_chunk @16384 (W73 KV_CHUNK_GROW), decode 256, stage timing, 80 GiB'; MTPLX_ROUTE_STAGE_PROBE=1 \$PY scripts/deepseek_v41/ab_decode_env_levers.py --context-tokens 16384 --decode-tokens 256 --max-kv 17408 --arms prefill_lean_sel_chunk --stage-timing --memory-limit-gib 80 --out $OUT/ar-16k-chunk.json 2>&1 | grep -vE \"\$F\" | tail -14
echo '[step 2b] DSpark cell16k @16384, decode 256, stage timing, 80 GiB'; MTPLX_ROUTE_STAGE_PROBE=1 \$PY scripts/deepseek_v41/ab_decode_env_levers.py --context-tokens 16384 --decode-tokens 256 --max-kv 17408 --decode-mode dspark --dspark-depth 3 --arms cell16k --stage-timing --memory-limit-gib 80 --out $OUT/dspark-16k-cell.json 2>&1 | grep -vE \"\$F\" | tail -14
echo '[window 29b done]'
"
