#!/bin/bash
# Window 35 (RUNNER FIRST): attribute the in-situ attention inflation — in-model census at T=1024 (same arm), at 16K with compile off, at 16K with win-memo off;
...
echo '[step 4] AR cell16k_ring_pool @16384 (W87 single slot pool) vs window-33 cell16k_ring 2.33'; MTPLX_ROUTE_STAGE_PROBE=1 \$PY \$AB --context-tokens 16384 --decode-tokens 256 --max-kv 17408 --arms cell16k_ring_pool --stage-timing --memory-limit-gib 60 --out $OUT/ar-ring-pool.json 2>&1 | grep -vE \"\$F\" | tail -20
