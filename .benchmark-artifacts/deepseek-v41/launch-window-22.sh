#!/bin/bash
# Window 22: 16K prefill arms round 2 (60 GiB plan): layer_major control, score_lean, dense_f32, dense_min32, prefill_lean.
...
OUT=$WT/docs/deepseek-v41/receipts/gpu-windows/window-22
mkdir -p "$OUT"
cd "$WT"
for i in $(seq 1 90); do curl -s -m 3 http://127.0.0.1:8080/health | grep -q '"ok":true' && break; sleep 10; done
echo "[launcher] $(date -u +%FT%TZ) agent healthy; starting window 22"
...
F='transformers\]|█|╗|╝|║|╭|╰|│'
echo '[step 1] 16K prefill arms round 2 (60 GiB plan)'; \$PY scripts/deepseek_v41/ab_decode_env_levers.py --context-tokens 16384 --decode-tokens 8 --max-kv 16640 --arms layer_major score_lean dense_f32 dense_min32 prefill_lean --prefill-stage-timing --memory-limit-gib 60 --out $OUT/prefill-16384-round2.json 2>&1 | grep -vE \"\$F\" | tail -40
echo '[window 22 done]'
"
